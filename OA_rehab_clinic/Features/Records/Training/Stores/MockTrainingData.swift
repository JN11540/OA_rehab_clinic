...
/**
 * TrainingDataManager - 訓練數據管理器
 * 負責整合真實數據和模擬數據
 */
class TrainingDataManager: ObservableObject {
    static let shared = TrainingDataManager()
    
    @Published private(set) var trainingResults: [TrainingVisualizationData] = []
    ...
    private func loadTrainingResults() {
        // 從 UserDefaults 或數據庫加載真實數據
        if let data = UserDefaults.standard.data(forKey: "trainingResults"),
           let decoded = try? JSONDecoder().decode([TrainingVisualizationData].self, from: data) {
            trainingResults = decoded
        }
    }
    
    private func saveTrainingResults() {
        if let encoded = try? JSONEncoder().encode(trainingResults) {
            UserDefaults.standard.set(encoded, forKey: "trainingResults")
        }
    }
    ...
    func getTrainingResults(
        ...
        if useMockData {
            // 使用模擬數據
            let allMockResults = MockTrainingData.generateMockTrainingResults(
                for: patientId,
                exerciseType: exerciseType
            )
            
            return allMockResults.filter { result in
            ...
            return trainingResults
                .filter { result in
                    ...
                    result.exerciseName == exerciseType.displayName &&
     ...
     * 添加新的訓練結果
     */
    func addTrainingResult(_ result: TrainingVisualizationData) {
